...
While active, this file is the scaffold plan only. It must also establish the repository document hierarchy so the many generated plan packs do not remain as competing active instructions. When this plan is fully implemented, move it to `archive/plans/implemented/`, rename it `YYYY-MM-DD-implemented-initrepoplan.md`, and add an implemented-state note at the top.
...
Parser provider parity source of truth is `collisionrelateddocs/Settings Backup/providers.json`, which currently contains 26 parser provider presets:
...
- Keep generated packs under a reference area such as `docs/reference/generated-packs/` or `archive/generated-packs/`; do not leave them looking like current implementation plans.
- Archive completed plans in `archive/plans/implemented/`.
- Archive superseded or merged plans in `archive/plans/superseded/` with a pointer to the replacement doc or ticket.
