...
Current verification baseline: `python -m pytest` and `python tools/run_parser_corpus.py --output docs/reference/data/parser_corpus_regression_report.json` pass with zero reader-level blockers across all 134 files under `docs/reference/raw/collisionrelateddocs/Instructions/`. Review-required field gaps remain explicit validation blockers where source data or provider rules do not supply mileage, VAT, mileage unit, inspection address, or VRM.

Review hardening on 2026-05-25 resolved the `tests/parsertests/output1.json` CNX engineer-report gap by adding deterministic fallback extraction after provider-rule extraction, suppressing legacy fixed-position `.DOC` noise, normalizing hyphenated month dates, deriving provider codes for engineer presets, hard-blocking EVA export for non-instruction evidence sources, routing email attachments through triage/readers, requiring full provider names for duplicate-code overrides, and restoring Tk drag/drop through `tkinterdnd2`. The repeat hardening iteration on 2026-05-25 added optional local Tesseract discovery for short image-only instruction PDFs, MP OCR extraction coverage, folder-level instruction plus engineer-report merging, engineer-report override warnings, per-parse attachment workspaces, partial batch failure reporting, source-hash reuse, and bounded external DOC converter timeouts. Remaining blockers are review-required data gaps or evidence/image-pack sources that should not export to EVA.
...
- Repeat iteration findings resolved after PR and corpus review: short MP image-only instruction PDFs are OCR-readable with the local Tesseract bundle and now populate MP provider, VRM, dates, claimant, reference, and inspection address; folder parsing now merges one instruction with one engineer report without replacing the instruction work provider; non-blank engineer-report overrides produce warnings; attachment extraction no longer reuses a shared deterministic temp directory; batch parsing reports per-item failures without discarding successes; DOC converter subprocesses have timeouts.
- Repeat iteration findings intentionally left as review-required: TEN 01/02 contain client contact instructions but no stored-at/address block; AMS 01 contains a client address but no configured vehicle location, so using it as the inspection site would be an unsafe inference under the inspection-site policy.
