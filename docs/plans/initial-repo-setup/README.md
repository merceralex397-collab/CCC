...
| - [x] Add and install a local project navigation/task-start skill | `initial-repo-setup` | `AGENTS.md`, `docs/docs_index.md`, `docs/repo_map.json`, `docs/plans/initial-repo-setup/local-repo-task-start-skill-plan.md`, local Codex skill `ccc-repo-task-start` | A scoped plan explains when a CCC repo-development skill should exist, what it should read first, how it avoids duplicating `AGENTS.md`, and why it is separate from production Collision Engineers skills in `docs/plans/agent-skills/`. The local skill is installed for Codex in the user's local skills home as `ccc-repo-task-start`. |
...
- [x] Create and install a local CCC repo task-start/navigation skill in the user's local Codex skills home as `ccc-repo-task-start`.
