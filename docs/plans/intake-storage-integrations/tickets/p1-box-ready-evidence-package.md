...
Image ordering rule from `claudechat.md`: two preview images followed by all images including the previews again. Network storage root is `\\192.168.0.106\Collision Engineers\` (see `handover.docx`). P1 generates local packages only — live Box upload is P3.
