...
| Mode | Use for | Output |
| --- | --- | --- |
| `market-valuation` | Establishing a defensible pre-accident retail value from live market evidence. | Valuation reasoning and assessed retail value. |
| `evidence-pack` | Producing the advert evidence PDF that supports a valuation. | Evidence pack PDF plus source table. |
| `valuation-explanation` | Explaining an assessed value to a client, solicitor, insurer, or engineer. | Plain-English explanation draft. |
| `dispute-response` | Responding to a guide-value, CAP/Cazana/Glass's/HPI, or settlement challenge. | AI-assisted dispute-response draft. |

Use `ce-house-style` for visual layout and writing tone.
...
4. Read these references before drafting the payload:
   ...
   - `../ce-house-style/references/visual-layout.md` when rendering PDFs
   - `../ce-house-style/references/writing-tone.md` when drafting external wording
